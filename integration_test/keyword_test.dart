...
void main() {
  patrolTest('Test Case 6 – Keywords Navigation', ($) async {
    ...
    // Navigate into Keywords sub-tab (icon: tag)
    await $.tester.tap(find.byIcon(Icons.tag).first);
    await $.tester.pumpAndSettle();

    // 2. Verify keyword statistics and keyword list are displayed.
    await $(RegExp(r'Keywords')).waitUntilVisible();
    expect(find.text('Keyword Frequency'), findsOneWidget);
